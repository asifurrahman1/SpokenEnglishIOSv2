  ...
  /*
   func fetchlesson(){
    ...
    print()
        ...
        var day = PerDay()
        for(var d = 1;d<6;d++){
        var links = [Tutorial_Obj]()
        for(var x=0;x<=2;x++){
           var l = Tutorial_Obj()
           l.TutorialName = self.data?[cnt].name
           l.Tutorial_url = self.data?[cnt].url
           links.append(l)
        }
        day.DayNo = d
        day.Days_tutorials = links
        weekday.append(day)
        }
        self.weeks?[w].WeekNo=w
        self.weeks?[w].days = weekday
        }
